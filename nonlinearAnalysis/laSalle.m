...
syms thetaS theta_dotS

%is it C^1 ?
V_sym = subs( V, [ theta theta_dot ], [ thetaS theta_dotS ]);

%with respect to each (theta, theta_dot)
simplify( diff(V_sym,thetaS) )
simplify( diff(V_sym,theta_dotS) )

%with respect to both (theta, theta_dot)
simplify( diff(diff(V_sym,thetaS),theta_dotS) )
