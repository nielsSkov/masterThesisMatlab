...
\item Programmerne kan bruges som en {\blue Matlab Toolbox}.
...
\begin{itemstep}
\item Program- og data-filnavnene indeholder information om programtype og navnet på den aktuelle proces. Begyndelsesbogstaverne
  indikerer typen:
\begin{itemize}
...
Performancefunktion: \ \ $\blue P(\theta) = \frac{1}{2N} \sum_{k=1}^{N}
\epsilon^2(k,\theta)$
...
hvor $\blue\epsilon_0$ er {\blue støj og undermodellering}, $\blue\epsilon_p$
er det {\blue parameter-afhængige} bidrag:
...
$\displaystyle\blue\epsilon_{p,RMS}(\theta) = \sqrt{\frac{1}{N} \sum_{k=1}^N
  \epsilon_{p}^2(k,\theta)} \approx \sqrt{\frac{1}{N} \sum_{k=1}^N
  (\theta - \theta_{N})^\top \psi^\top\psi (\theta -
  \theta_{N})}$}\\[1ex]
\hspace*{0.4mm}$\blue = \sqrt{(\theta - \theta_{N})^\top \tilde{H} (\theta_{N})(\theta -
 \theta_{N} )}$,
med $\tilde{H} (\theta) = \frac{1}{N} \sum_{k=1}^N \psi^\top(k,\theta)\psi(k,\theta)$.

}%
\fromSlide*{2}{%
\untilSlide*{3}{%
Vi skalerer mht. $y$ og $\theta_N$: $L=diag(\theta_N)$, $\theta_r = L^{-1} \theta$.
\hspace*{-1em}\mbox{\small
...
\hspace*{15.4mm}$\blue = \sqrt{(\theta_r - 1_v)^\top \tilde{H}_{rn} (\theta_r -
  1_v)}$\\[1ex]
med \:\ $\psi_{rn}(k,\theta) = y_{RMS}^{-1} L \psi(k,\theta)$, 
\:\ $\theta_{r,i} = \frac{\theta_i}{\theta_{N,i}}$.
}%
\onlySlide*{3}{%

\bigskip\bigskip
{\blue Parameterfølsomhed} m.h.t. den $i$'te parameter $\theta_i$:
...
%---------------------------------------------------------------------- SLIDE -
\overlays{4}{
\begin{slide}[Replace]{Følsomhedsellipsoide}

\onlySlide*{1}{%
 Eksempel med to parametre: \quad $\blue\theta = \begin{bmatrix} \theta_1\\
  \theta_2 \end{bmatrix}$
...
\bigskip
I sådan en ellipse er afstanden fra (0,0) til punktet $(x_i,0)$ på ellipsen
$= \frac{c}{\sqrt{h_{ii}}}$
}%
\onlySlide*{2}{%
