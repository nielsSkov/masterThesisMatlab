\message{ !name(sim_exp-slides4.tex)}{\makeatletter\gdef\AucTeX@cite#1[#2]#3{[#3#1#2]}\gdef\cite{\@ifnextchar[{\AucTeX@cite{, }}{\AucTeX@cite{}[]}}}

\message{ !name(sim_exp-slides4.tex) !offset(-3) }
...
\hspace*{150pt}{\tiny\red støj \quad model fejl \quad par.
...
idet $\blue\epsilon_x$ og $\blue\epsilon_m$ er uafhængige.
}%
...
%---------------------------------------------------------------------- SLIDE -
\overlays{6}{
\begin{slide}[Replace]{Parameter følsomhed og nøjagtighed}

\untilSlide*{3}{
Brug følsomhedsmålene: $\blue S_i$, $\blue S_{i\text{ min}}$, $\blue R =
S_\text{max}/S_\text{min}$, $\blue R_i = S_i/S_{i\text{ min}}$
til at evaluere situationen, fx. nødvendighed af ændret model struktur
eller input.

\fromSlide*{2}{%
\bigskip
Parameter nøjagtigheden kan udtrykkes ved\\
Kun støj:
...
%---------------------------------------------------------------------- SLIDE -
\overlays{2}{
\begin{slide}[Replace]{Eksempel: Højtaler}
...
Indput signalet skal have størstedelen af effekten i frekvensområdet,
hvor nøjagtigh. af modellen er vigtigst.

\fromSlide{2}{%
\medskip
{\blue Ulineære systemer}:\\
Amplitude variationen i input signalet skal svare til det amplitude
område, hvor nøjagtigh. af modellen er vigtigst.
Indput signalet skal have størstedelen af effekten i frekvensområdet,
hvor nøjagtigh. af modellen er vigtigst.
}
...
\bigskip
{\red Procedure}:\\
Design et input signal der optimerer følsomhedsmålene.
}%
...
\begin{itemstep}
\item[\red 1.] {\blue Bestem tilnærmede parameter værdier} eller find á priori
  parameter værdier.
\item[\red 2.] {\blue Vælg en klasse af input signaler} -- skal afhænge af få
...
\item[\red 3.] {\blue Optimer input signalet til bedste følsomhed
  ...
  fkt. af input signal parametre, vælg bedste værdi for disse.
\item[\red 4.] {\blue Brug det fundne input signal på det fysiske system}.\\
Hvis nødvendigt, gentag proceduren med de forbedrede parameter
estimater.
\end{itemstep}
...
\message{ !name(sim_exp-slides4.tex) !offset(-300) }
